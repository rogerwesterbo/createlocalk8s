...
_k8s_local_completion() {
    ...
    _k8s_local_get_helm_components() {
        ...
        if [[ -z "$_K8S_LOCAL_HELM" ]]; then
            ...
            [[ -z "$_K8S_LOCAL_HELM" ]] && _K8S_LOCAL_HELM="argocd cert-manager cnpg crossplane falco hashicorp-vault kube-prometheus-stack kubeview metallb metrics-server minio mongodb-operator nats nfs nginx-ingress opencost pgadmin prometheus rook-ceph-operator trivy valkey"
        fi
    ...
    _k8s_local_get_argo_apps() {
        ...
        if [[ -z "$_K8S_LOCAL_APPS" ]]; then
            ...
            [[ -z "$_K8S_LOCAL_APPS" ]] && _K8S_LOCAL_APPS="nyancat prometheus cert-manager cnpg-cluster crossplane falco hashicorp-vault kubeview metallb metrics-server minio mongodb mongodb-operator nats nfs opencost pg-ui pgadmin rook-ceph-cluster rook-ceph-operator trivy coredns valkey"
        fi

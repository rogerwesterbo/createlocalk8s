...
-   Supports OIDC, SAML, OAuth 2.0 for modern authentication/authorization flows.
-   Locally: practice integrating applications with SSO, testing social login flows (Google, GitHub, etc.), managing user realms and roles.
-   Auto-configured with PostgreSQL backend for production-like setup.
-   Default admin credentials: `admin/admin` (change in production!).
-   Access via ingress at `http://keycloak.localtest.me` or port-forward: `kubectl port-forward -n keycloak svc/keycloak-keycloakx-http 15003:80`.
